...
public class Board {
    ...
    //Arraylist used for creating the board using the minum number of player to put an Itemcard in the Cell
    ...
    public void checkRefill(int NumofPlayers){
        ...
        for(int i=0;i< board.length && refill;i++) {
            for (int j = 0; j < board[0].length && refill; j++) {
                if((i==0||i==8)||(j==0||j==8) && board[i][j].itemCard!=null) refill=checkSide(i,j);
                else if(board[i][j].itemCard!=null) refill=checkPosition(i,j);
            }
        }
        System.out.println("REFILLING THE BOARD");
        if(refill) fillBoard(NumofPlayers);
    }
    public boolean checkSide(int i,int j){
        if(i==0){
            if (board[i+1][j].itemCard!=null||board[i][j+1].itemCard!=null||board[i+1][j-1].itemCard!=null) return false;
        }
        if(i==8){
            if (board[i-1][j].itemCard!=null||board[i][j+1].itemCard!=null||board[i+1][j-1].itemCard!=null) return false;
        }
