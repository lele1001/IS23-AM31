...
public class Board {
    ...
    public void checkRefill(int NumofPlayers){
        ...
        for(int i=0;i< board.length && refill;i++) {
            for (int j = 0; j < board[0].length && refill; j++) {
                if(((i==0||i==8)||(j==0||j==8)) && board[i][j].itemCard!=null && board[i][j].numPlayermin<=NumofPlayers) refill=checkSide(i,j);
                else if(board[i][j].itemCard!=null&&board[i][j].numPlayermin<=NumofPlayers) refill=checkPosition(i,j);
            }
        }
        if(refill) {
    ...
    public boolean checkSide(int i,int j){
        if(i==0){
            if (board[i+1][j].itemCard!=null||board[i][j+1].itemCard!=null||board[i][j-1].itemCard!=null) return false;
        }
        if(i==8){
            if (board[i-1][j].itemCard!=null||board[i][j+1].itemCard!=null||board[i][j-1].itemCard!=null) return false;
        }
        if(j==0){
            if (board[i+1][j].itemCard!=null||board[i-1][j].itemCard!=null||board[i][j+1].itemCard!=null) return false;
        }
        if(j==8){
            if (board[i+1][j].itemCard!=null||board[i-1][j].itemCard!=null||board[i][j-1].itemCard!=null) return false;
        }
    ...
    public ArrayList<ItemCard> deleteSelection(ArrayList<Integer> Position){
        ...
        for(int pos : Position){
            if(board[pos / 10][pos % 10].itemCard==null) System.out.println("Not find card at"+ pos / 10 + " " + pos % 10);
